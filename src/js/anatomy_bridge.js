// anatomy_bridge.js
// Integra el SVG corporal y lo enlaza con focusZoneCard(zoneName)
// Multi-selección con Ctrl/⌘. Botón limpiar: #btnClearAnatomy

(() => {
  const HOST_ID = "anatomyMount";

  // Mapeo SVG -> ZONAS del cuestionario (responder_cuestionario.js)
  const MAP_SVG_TO_ZONE = {
    "head": ["Cuello"],
    "left-shoulder": ["Hombro Izquierdo"],
    "right-shoulder": ["Hombro Derecho"],
    "left-arm": ["Codo/antebrazo Izquierdo"],
    "right-arm": ["Codo/antebrazo Derecho"],
    "left-hand": ["Muñeca/mano Izquierda"],
    "right-hand": ["Muñeca/mano Derecha"],
    "chest": ["Espalda Alta"],
    "stomach": ["Espalda Baja"],
    "left-leg": ["Caderas/nalgas/muslos", "Rodillas (una o ambas)"],
    "right-leg": ["Caderas/nalgas/muslos", "Rodillas (una o ambas)"],
    "left-foot": ["Pies/tobillos (uno o ambos)"],
    "right-foot": ["Pies/tobillos (uno o ambos)"]
  };

  // Fallback inline por si no se puede fetch() del HTML externo
  const HUMAN_BODY_INLINE = `
  ...
  async function boot() {
    ...
    // 1) Intentar cargar el HTML externo si se definió ANATOMY_HTML_PATH
    if (window.ANATOMY_HTML_PATH) {
      try {
        const res = await fetch(String(window.ANATOMY_HTML_PATH), { cache: "no-store" });
        if (res.ok) {
          const html = await res.text();
          const tmp = document.createElement("div");
          tmp.innerHTML = html;
          const body = tmp.querySelector(".human-body");
          if (body) {
            host.innerHTML = "";
      ...
      } catch { /* continua al fallback */ }
    }

    // 2) Fallback inline
    host.innerHTML = HUMAN_BODY_INLINE;
    wireBody(host);
    wireClear(host);
  }

  function wireBody(root) {
    root.querySelectorAll(".human-body svg").forEach(svg => {
      ...
      const onSelect = (ev) => {
        ...
        if (!multi) clearSelection(root);

        const pressed = svg.getAttribute("aria-pressed") === "true";
        svg.setAttribute("aria-pressed", pressed ? "false" : "true");

        const key = svg.id || svg.getAttribute("data-position") || "";
        const zones = MAP_SVG_TO_ZONE[key] || [];
        zones.forEach(z => {
          if (typeof window.focusZoneCard === "function") window.focusZoneCard(z);
        });
      };

      svg.addEventListener("click", onSelect);
  ...
  function clearSelection(root) {
    root.querySelectorAll(".human-body svg[aria-pressed='true']").forEach(svg => {
      svg.setAttribute("aria-pressed", "false");
    });
  }

  function wireClear(root) {
    const btn = document.getElementById("btnClearAnatomy");
    if (!btn) return;
    btn.addEventListener("click", () => clearSelection(root));
  }
